...
describe('ImageProcessingService', () => {
  ...
  describe('processAndSaveImage', () => {
    it('should process and save a valid image', async () => {
      ...
      const result = await imageProcessingService.processAndSaveImage(
        testBuffer,
        'test.jpg',
        { maxWidth: 400, maxHeight: 300, quality: 80 }
      );
  ...
  describe('createImageVariants', () => {
    it('should create multiple image variants', async () => {
      ...
      const results = await imageProcessingService.createImageVariants(
        testBuffer,
        'test.jpg',
        variants
      );
  ...
  describe('optimizeExistingImage', () => {
    it('should optimize an existing image with new settings', async () => {
      ...
      const original = await imageProcessingService.processAndSaveImage(
        testBuffer,
        'optimize-test.jpg',
        { quality: 100 }
      );

      // Optimize with lower quality and smaller size
      const optimized = await imageProcessingService.optimizeExistingImage(
        original.id,
        { maxWidth: 500, maxHeight: 400, quality: 60, format: 'webp' }
      );

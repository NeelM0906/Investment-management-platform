import { DealRoomModel, DealRoomCreateData } from './DealRoom';

describe('DealRoomModel', () => {
  describe('validate', () => {
    it('should validate a valid deal room data', () => {
      const validData: DealRoomCreateData = {
      ...
      const result = DealRoomModel.validate(validData);
  ...
  describe('isValidUrl', () => {
    it('should validate correct URLs', () => {
      expect(DealRoomModel.isValidUrl('https://example.com')).toBe(true);
      expect(DealRoomModel.isValidUrl('http://example.com')).toBe(true);
      expect(DealRoomModel.isValidUrl('https://example.com/path?query=1')).toBe(true);
    });

    it('should reject invalid URLs', () => {
      expect(DealRoomModel.isValidUrl('not-a-url')).toBe(false);
      expect(DealRoomModel.isValidUrl('ftp://example.com')).toBe(true); // FTP is valid URL
      expect(DealRoomModel.isValidUrl('')).toBe(false);
      expect(DealRoomModel.isValidUrl('example.com')).toBe(false); // Missing protocol
    });
  });

  describe('isValidImageMimeType', () => {
    it('should validate correct image MIME types', () => {
      expect(DealRoomModel.isValidImageMimeType('image/jpeg')).toBe(true);
      expect(DealRoomModel.isValidImageMimeType('image/jpg')).toBe(true);
      expect(DealRoomModel.isValidImageMimeType('image/png')).toBe(true);
      expect(DealRoomModel.isValidImageMimeType('image/webp')).toBe(true);
      expect(DealRoomModel.isValidImageMimeType('IMAGE/JPEG')).toBe(true); // Case insensitive
    });

    it('should reject invalid MIME types', () => {
      expect(DealRoomModel.isValidImageMimeType('image/gif')).toBe(false);
      expect(DealRoomModel.isValidImageMimeType('text/plain')).toBe(false);
      expect(DealRoomModel.isValidImageMimeType('application/pdf')).toBe(false);
      expect(DealRoomModel.isValidImageMimeType('')).toBe(false);
    });
  });
  ...
  describe('createDefault', () => {
    it('should create a default deal room', () => {
      ...
      const dealRoom = DealRoomModel.createDefault(projectId);
  ...
  describe('generateId', () => {
    it('should generate unique IDs', () => {
      const id1 = DealRoomModel.generateId();
      const id2 = DealRoomModel.generateId();
  ...
  describe('generateItemId', () => {
    it('should generate unique item IDs', () => {
      const id1 = DealRoomModel.generateItemId();
      const id2 = DealRoomModel.generateItemId();

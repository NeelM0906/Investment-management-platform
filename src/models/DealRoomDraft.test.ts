import { DealRoomDraftModel, DealRoomDraftCreateData, DealRoomDraft } from './DealRoomDraft';

describe('DealRoomDraftModel', () => {
  describe('validate', () => {
    it('validates a valid draft creation data', () => {
      const validData: DealRoomDraftCreateData = {
      ...
      const result = DealRoomDraftModel.validate(validData);
    ...
    it('requires project ID', () => {
      ...
      const result = DealRoomDraftModel.validate(invalidData);
    ...
    it('validates investment blurb type', () => {
      const invalidData: any = {
        ...
        sessionId: 'session-456',
        draftData: {
          investmentBlurb: 123 // Should be string
        },
        isAutoSave: true
      };
    ...
    it('validates investment blurb length', () => {
      const invalidData: DealRoomDraftCreateData = {
        projectId: 'project-123',
        sessionId: 'session-456',
        draftData: {
          investmentBlurb: 'a'.repeat(501) // Too long
        },
        isAutoSave: true
      };
    ...
    it('validates investment summary type', () => {
      const invalidData: any = {
        ...
        sessionId: 'session-456',
        draftData: {
          investmentSummary: 123 // Should be string
        },
        isAutoSave: true
      };
    ...
    it('validates investment summary length', () => {
      const invalidData: DealRoomDraftCreateData = {
        ...
        sessionId: 'session-456',
        draftData: {
          investmentSummary: 'a'.repeat(10001) // Too long
        },
        isAutoSave: true
      };
  ...
  describe('isValidUrl', () => {
    it('validates correct URLs', () => {
      expect(DealRoomDraftModel.isValidUrl('https://example.com')).toBe(true);
      expect(DealRoomDraftModel.isValidUrl('http://example.com')).toBe(true);
      expect(DealRoomDraftModel.isValidUrl('https://example.com/path?query=1')).toBe(true);
      expect(DealRoomDraftModel.isValidUrl('ftp://example.com')).toBe(true);
    });

    it('rejects invalid URLs', () => {
      expect(DealRoomDraftModel.isValidUrl('not-a-url')).toBe(false);
      expect(DealRoomDraftModel.isValidUrl('')).toBe(false);
      expect(DealRoomDraftModel.isValidUrl('example.com')).toBe(false); // Missing protocol
      expect(DealRoomDraftModel.isValidUrl('://example.com')).toBe(false); // Invalid protocol
    });
  });

  describe('generateId', () => {
    it('generates unique IDs with correct prefix', () => {
      const id1 = DealRoomDraftModel.generateId();
      const id2 = DealRoomDraftModel.generateId();
  ...
  describe('generateSessionId', () => {
    it('generates unique session IDs with correct prefix', () => {
      const id1 = DealRoomDraftModel.generateSessionId();
      const id2 = DealRoomDraftModel.generateSessionId();
  ...
  describe('generateConflictId', () => {
    it('generates unique conflict IDs with correct prefix', () => {
      const id1 = DealRoomDraftModel.generateConflictId();
      const id2 = DealRoomDraftModel.generateConflictId();
  ...
  describe('createDefault', () => {
    it('creates a default draft with correct properties', () => {
      ...
      const draft = DealRoomDraftModel.createDefault(projectId, sessionId);
      ...
      // Check that expiration is 24 hours from creation
  ...
  describe('detectConflicts', () => {
    it('detects no conflicts when data is identical', () => {
      ...
      const conflicts = DealRoomDraftModel.detectConflicts(data1, data2);
    ...
    it('detects conflicts in investment blurb', () => {
      ...
      const conflicts = DealRoomDraftModel.detectConflicts(localData, serverData);
  ...
  describe('mergeData', () => {
    ...
    it('uses local data when resolution is use_local', () => {
      const merged = DealRoomDraftModel.mergeData(localData, serverData, 'use_local');
      expect(merged).toEqual(localData);
    });

    it('uses server data when resolution is use_server', () => {
      const merged = DealRoomDraftModel.mergeData(localData, serverData, 'use_server');
      expect(merged).toEqual(serverData);
    });

    it('merges data when resolution is merge', () => {
      const merged = DealRoomDraftModel.mergeData(localData, serverData, 'merge');
      
      expect(merged.investmentBlurb).toBe('Local blurb'); // From local
      expect(merged.investmentSummary).toBe('Server summary'); // From server
      expect(merged.keyInfo).toEqual(localData.keyInfo); // From local
      expect(merged.externalLinks).toEqual(serverData.externalLinks); // From server
    });

    it('prefers local data in merge when both have same field', () => {
      const localWithSummary = { ...localData, investmentSummary: 'Local summary' };
      const merged = DealRoomDraftModel.mergeData(localWithSummary, serverData, 'merge');
      
      expect(merged.investmentSummary).toBe('Local summary'); // Local takes precedence
    });

    it('defaults to local data for unknown resolution', () => {
      const merged = DealRoomDraftModel.mergeData(localData, serverData, 'unknown' as any);
      expect(merged).toEqual(localData);
    });
  });
});
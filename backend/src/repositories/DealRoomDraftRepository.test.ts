...
import { DealRoomDraftRepository } from './DealRoomDraftRepository';
import { DealRoomDraft, DealRoomDraftCreateData, DealRoomVersion, ConflictResolution } from '../models/DealRoomDraft';
...
describe('DealRoomDraftRepository', () => {
  let repository: DealRoomDraftRepository;
  const testDataDir = path.join(process.cwd(), 'data');

  beforeEach(() => {
    repository = new DealRoomDraftRepository();
    jest.clearAllMocks();
  });
  ...
  describe('ensureDataFilesExist', () => {
    it('creates data files if they do not exist', async () => {
      ...
      expect(mockFs.writeFile).toHaveBeenCalledTimes(3); // drafts, versions, conflicts
      expect(mockFs.writeFile).toHaveBeenCalledWith(
        path.join(testDataDir, 'deal-room-drafts.json'),
  ...
  describe('readDrafts', () => {
    ...
    it('filters out expired drafts', async () => {
      ...
      const valid = new Date(now.getTime() + 1000); // 1 second from now
      ...
      expect(mockFs.writeFile).toHaveBeenCalled(); // Should save cleaned data
    });
  ...
  describe('createDraft', () => {
    ...
    it('updates existing draft when one exists', async () => {
      ...
      expect(result.lastSavedVersion).toBe(1); // Preserved
    });
  });
  ...
  describe('updateDraft', () => {
    ...
    it('throws error when draft not found', async () => {
      ...
      await expect(
        repository.updateDraft('project-1', 'session-1', {})
  ...
  describe('cleanupExpiredDrafts', () => {
    it('removes expired drafts and returns count', async () => {
      ...
      expect(mockFs.writeFile).toHaveBeenCalledWith(
        path.join(testDataDir, 'deal-room-drafts.json'),
        expect.stringContaining('draft-valid')
      );
    });
  ...
  describe('createVersion', () => {
    ...
    it('limits versions to 10 per project', async () => {
      ...
      // Should have called writeVersions with cleaned data
      const writeCall = (mockFs.writeFile as jest.Mock).mock.calls.find(
        call => call[0].includes('deal-room-versions.json')
      );
      expect(writeCall).toBeDefined();
      
      const savedVersions = JSON.parse(writeCall[1]);
      const projectVersions = savedVersions.filter((v: any) => v.projectId === 'project-1');
  ...
  describe('getVersionsByProject', () => {
    ...
    it('respects limit parameter', async () => {
      ...
      expect(result[0].version).toBe(15); // Highest version first
    });
  });
  ...
  describe('resolveConflict', () => {
    ...
    it('throws error when conflict not found', async () => {
      ...
      await expect(
        repository.resolveConflict('conflict-123', 'use_local')
    ...
    it('returns only unresolved conflicts for project', async () => {
      const mockConflicts = [
        ...
        },
        {
          ...
          conflictFields: [],
          createdAt: '2023-01-01T00:00:00.000Z',
          resolvedAt: '2023-01-01T01:00:00.000Z' // Resolved
        },
        {

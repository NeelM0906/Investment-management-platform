...
const EditProjectPage: React.FC = () => {
      ...
      try {
        ...
        if (data.success) {
          ...
          setFormData({
            ...
            legalProjectName: project.legalProjectName,
            unitCalculationPrecision: project.unitCalculationPrecision,
            targetAmount: project.targetAmount,
            minimumInvestment: project.minimumInvestment,
            currency: project.currency,
            startDate: project.timeframe.startDate.split('T')[0],
            endDate: project.timeframe.endDate.split('T')[0]
          });
        } else {
          setErrors({ general: data.error?.message || 'Failed to fetch project' });
        }
      } catch (err) {
        setErrors({ general: 'Failed to fetch project. Make sure the server is running.' });
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        alert(`Project "${data.data.projectName}" updated successfully!`);
        navigate(`/projects/${id}`);
      } else {
        setErrors({ general: data.error?.message || 'Failed to update project' });
      }
    } catch (error) {
      setErrors({ general: 'Failed to update project. Make sure the server is running.' });
    } finally {
      setSaving(false);
    }
  };
    ...
    <div>
      <div className="page-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1rem' }}>
          <Link to={`/projects/${id}`} className="btn btn-secondary" style={{ padding: '0.5rem' }}>
            <ArrowLeft size={16} />
          </Link>
          <div>
            <h1 className="page-title">Edit Project</h1>
            <p className="page-subtitle">Update project information</p>
          </div>
        </div>
      </div>
